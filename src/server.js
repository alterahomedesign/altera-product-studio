...
app.post('/api/parse-catalog', upload.single('file'), async(req,res)=>{
  try{
    ...
    if(isExcel){
      ...
      // PDF — extract images with pdf-lib + quotation-aware parsing
...
function fillProductRow(r,prod,v,isFirst){
  ...
  if(isFirst){
    ...
    if(v.dimsF) r['Dimensions (product.metafields.custom.dimensions)']=v.dimsF;
  }
